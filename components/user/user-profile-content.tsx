import Image from "next/image";
import Role from "@/components/user/role";
import { Calendar } from "lucide-react";
import { formatDate } from "@/lib/utils";
import { User } from "@/sanity.types";
import { Suspense } from "react";
import UserProfileStats from "./user-profile-stats";
import UserPosts from "./user-posts";
import { SortOrder } from "@/types/filter";
import SortFilterSelect from "../sort-filter-select";
import UserActionsWrapper from "./user-actions-wrapper";

type UserProfileContentProps = {
  targetUser: User | null;
  sort: SortOrder;
};

const UserProfileContent = async ({
  targetUser,
  sort,
}: UserProfileContentProps) => {
  return (
    <section className="flex flex-col w-full max-w-[70rem] mx-auto px-4 sm:px-6 py-6 sm:py-8 md:py-10 gap-6 md:gap-12">
      <div className="flex flex-row w-full items-start justify-center sm:justify-between">
        <div className="flex flex-row items-center gap-4 sm:gap-6">
          <Image
            src={targetUser?.imageUrl || "/avatar-default.svg"}
            alt={`${targetUser?.username}'s avatar`}
            className="size-24 sm:size-36 md:w-48 md:h-48 rounded-full object-cover border-4 border-background shadow-md"
            width={192}
            height={192}
            priority
            placeholder="blur"
            blurDataURL="/avatar-default.svg"
          />

          <div className="flex flex-col text-left">
            <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold flex flex-wrap items-center justify-start gap-2">
              {targetUser?.fullName || targetUser?.username || "Unknown user"}{" "}
              <Role role={targetUser?.role} />
            </h1>

            <p className="text-muted-foreground text-base">
              @{targetUser?.username || "unknown-user"}
            </p>

            <p
              className={`${
                targetUser?.createdAt ? "flex" : "hidden"
              } text-muted-foreground items-center text-sm gap-1 justify-center sm:justify-start`}
              title={`Joined on ${formatDate(targetUser?.createdAt)}`}
            >
              <Calendar className="h-4 w-4" /> Joined on{" "}
              {formatDate(targetUser?.createdAt)}
            </p>

            <p className="text-sm md:text-base text-muted-foreground mt-2">
              {targetUser?.bio || "This user has not set a bio yet."}
            </p>
            <div className="hidden sm:block">
              <Suspense
                fallback={
                  <div className="h-16 w-full bg-background animate-pulse rounded-md" />
                }
              >
                <UserProfileStats user={targetUser} />
              </Suspense>
            </div>
          </div>
        </div>

        <div className="hidden sm:flex flex-col gap-6 mt-0 md:mt-8">
          <UserActionsWrapper user={targetUser} />
        </div>
      </div>
      <div className="sm:hidden flex flex-col gap-6">
        <UserActionsWrapper user={targetUser} />
        <Suspense
          fallback={
            <div className="h-16 w-full bg-background animate-pulse rounded-md" />
          }
        >
          <UserProfileStats user={targetUser} />
        </Suspense>
      </div>
      <div className="flex flex-col gap-6 mt-10">
        <div className="flex flex-col md:flex-row items-center md:items-end justify-between gap-3 md:gap-0">
          <div className="flex flex-col items-center md:items-start">
            <h2 className="text-xl sm:text-2xl font-bold">Latest Activity</h2>
            <p className="text-sm sm:text-base text-muted-foreground">
              Here are the latest posts made by{" "}
              {targetUser?.username || "Unknown user"}.
            </p>
          </div>

          <SortFilterSelect />
        </div>

        <UserPosts user={targetUser} sort={sort} />
      </div>
    </section>
  );
};

export default UserProfileContent;
